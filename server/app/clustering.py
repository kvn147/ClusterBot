...
class PostClusterer:
    def __init__(self, similarity_threshold: float = 0.25):  # Threshold that should capture all similar posts
    ...
    def preprocess_text(self, title: str, content: str = "") -> str:
        ...
        text = re.sub(r'\bupdate[sd]?\b', 'update', text, flags=re.IGNORECASE)  # Normalize updates/updating
        text = re.sub(r'\breport(?:s|ed|ing)?\b', 'report', text, flags=re.IGNORECASE)  # Normalize reports/reported
        text = re.sub(r'\bpower outages?\b', 'power outage', text, flags=re.IGNORECASE)  # Normalize power outage terms
        
        # Extract and preserve key info: locations, numbers, disaster types
        numbers = re.findall(r'\d+(?:\.\d+)?', text)  # Extract numbers like 7.2
        locations = re.findall(r'\b(?:japan|tokyo|honshu|osaka|kyoto|sendai|fukushima|hokkaido|okinawa)\b', 
                              text.lower())  # Common locations
        disaster_terms = re.findall(r'\b(?:earthquake|tsunami|aftershock|tremor|damage|evacuat(?:e|ion)|warning)\b', 
                                   text.lower())  # Key disaster terms
        
        # Remove Reddit-specific formatting and noise
        text = re.sub(r'\[.*?\]', '', text)  # Remove [brackets]
        text = re.sub(r'\(.*?\)', '', text)  # Remove (parentheses) 
        text = re.sub(r'breaking:?', '', text, flags=re.IGNORECASE)  # Remove "breaking"
        text = re.sub(r'live updates?:?', 'update', text, flags=re.IGNORECASE)  # Normalize "live updates"
        text = re.sub(r'http\S+', '', text)  # Remove URLs
        text = re.sub(r'[^\w\s]', ' ', text)  # Remove special chars
        text = re.sub(r'\s+', ' ', text).strip()  # Normalize whitespace
        
        # Add extracted key info back with higher weight (repeat them)
        key_info = " ".join([" ".join(locations)*3, " ".join(numbers)*3, " ".join(disaster_terms)*3])
    ...
    def find_similar_cluster(self, post: Dict) -> Optional[int]:
        ...
        for cluster_id, cluster_data in self.active_clusters.items():
            ...
            rep_text = self.preprocess_text(representative_post['title'], representative_post.get('selftext', ''))
            ...
            if keyword_overlap < 0.2:  # Skip detailed comparison for very low overlap
                debug_info.append(f"Cluster {cluster_id}: {keyword_overlap:.2f} keyword overlap (too low)")
                continue
            ...
            if post_domain and post_domain == cluster_data.get('domain', ''):
                ...
                if post_domain not in ['twitter.com', 'youtube.com', 'facebook.com', 'instagram.com', 'reddit.com']:
                    debug_info.append(f"Cluster {cluster_id}: Same domain {post_domain} -> MATCHED")
                    return cluster_id
            
            # Title-focused similarity matching
            title_similarity = self.calculate_title_similarity(post, representative_post)
            
            debug_info.append(f"Cluster {cluster_id}: {title_similarity:.3f} similarity")
            
            # Check for event-specific matches (keywords like earthquake, location, magnitude)
            event_match = self._check_event_match(post['title'], representative_post['title'])
            if event_match:
                title_similarity += 0.15  # Boost similarity for event matches
                debug_info.append(f"  ✓ Event match detected: +0.15 boost -> {title_similarity:.3f}")
    ...
    def _check_event_match(self, title1: str, title2: str) -> bool:
        ...
        event_types = ['earthquake', 'tsunami', 'hurricane', 'typhoon', 'tornado', 'flood', 'wildfire']
        event_match = False
        ...
        # Extract locations
        locations = ['japan', 'california', 'florida', 'texas', 'china', 'india', 'europe', 'australia']
    ...
    def _quick_keyword_overlap(self, text1: str, text2: str) -> float:
        ...
        important_keywords = {'earthquake', 'tsunami', 'japan', 'damage', 'warning', 'report', 'update'}
        ...
        important_matches = len([word for word in intersection if word in important_keywords])
