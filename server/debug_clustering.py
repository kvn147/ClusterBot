    ...
    try:
        ...
        for i in range(len(eq_posts)):
            for j in range(i + 1, len(eq_posts)):
                ...
                status = "✅ MATCH" if similarity > 0.8 else "⚠️ THRESHOLD" if similarity > 0.6 else "❌ NO MATCH"
...
def test_different_thresholds():
    ...
    for threshold in thresholds:
        ...
        score = "🎯 PERFECT!" if clustered_count == len(eq_posts) else "⚠️ Split" if clustered_count > 1 else "❌ All separate"
...
def main():
    ...
    if max_similarity and max_similarity < 0.8:
        print(f"🎯 Lower your similarity threshold from 0.8 to {max_similarity - 0.05:.2f}")
