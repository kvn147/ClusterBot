...
def test_clustering_with_sample_data():
    ...
    for i, post in enumerate(earthquake_posts[1:], 1):
        ...
        if cluster_id:
            ...
            print(
                f"✅ Post {i+1} matched to cluster {cluster_id}: '{post['title'][:50]}...'"
            )
            earthquake_clusters[cluster_id] = earthquake_clusters.get(cluster_id, 0) + 1
        else:
            new_cluster = clusterer.create_cluster(post)
            print(
                f"⚠️  Post {i+1} created NEW cluster {new_cluster}: '{post['title'][:50]}...'"
            )
    ...
    for i, post in enumerate(tech_posts):
        ...
        if cluster_id:
            clusterer.add_to_cluster(cluster_id, post)
            print(
                f"✅ Tech post {i+1} matched to cluster {cluster_id}: '{post['title'][:50]}...'"
            )
            tech_clusters[cluster_id] = tech_clusters.get(cluster_id, 0) + 1
        else:
            new_cluster = clusterer.create_cluster(post)
            print(
                f"🆕 Tech post {i+1} created cluster {new_cluster}: '{post['title'][:50]}...'"
            )
    ...
    for i, post in enumerate(unrelated_posts):
        ...
        if cluster_id:
            print(
                f"⚠️  Unrelated post {i+1} matched to existing cluster {cluster_id}: '{post['title'][:50]}...'"
            )
            unrelated_matches += 1
        else:
            new_cluster = clusterer.create_cluster(post)
            print(
                f"✅ Unrelated post {i+1} correctly created new cluster {new_cluster}: '{post['title'][:50]}...'"
            )

    print(
        f"📊 Unrelated posts incorrectly clustered: {unrelated_matches}/{len(unrelated_posts)}"
    )
    ...
    if duplicate_posts:
        ...
        print(
            f"🆕 Created cluster {dup_cluster_id} for: '{first_dup['title'][:50]}...'"
        )
        ...
        for i, post in enumerate(duplicate_posts[1:], 1):
            ...
            if cluster_id == dup_cluster_id:
                print(
                    f"✅ Duplicate URL post {i+1} correctly matched: '{post['title'][:50]}...'"
                )
            elif cluster_id:
                print(
                    f"⚠️  Duplicate URL post {i+1} matched wrong cluster {cluster_id}: '{post['title'][:50]}...'"
                )
            else:
                print(
                    f"❌ Duplicate URL post {i+1} failed to cluster: '{post['title'][:50]}...'"
                )
    ...
    for cluster_id, cluster_data in clusterer.active_clusters.items():
        print(
            f"Cluster {cluster_id}: {cluster_data['post_count']} posts - '{cluster_data['title'][:40]}...'"
        )
...
def test_preprocessing():
    ...
    test_cases = [
        (
            "Breaking: Major [UPDATE] earthquake hits Japan (7.2 magnitude)",
            "earthquake japan",
        ),
