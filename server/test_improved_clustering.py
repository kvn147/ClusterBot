...
def test_earthquake_clustering():
    ...
    if len(cluster_assignments) == 1:
        ...
        print(f"⚠️ Not ideal. Posts were split into {len(cluster_assignments)} clusters.")
...
def test_different_thresholds():
    ...
    for threshold, data in results.items():
        ...
        distribution = ', '.join([f"Cluster {c}: {n} posts" for c, n in data['distribution'].items()])
    ...
    if best_cluster_count == 1:
        ...
        print(f"📝 Consider further tuning the algorithm to achieve single-cluster grouping.")
